...
export interface WorkbenchOptions {
  ...
  maxQueues?: number;
  /**
   * Optional callbacks that let the host app handle scheduler edits via its
   * own control plane instead of mutating BullMQ directly. Required if you
   * want the Schedulers UI to expose edit / disable / run-now actions —
   * Workbench leaves the actions off when this is not provided, since direct
   * `removeRepeatableByKey` writes get clobbered by hosts that reconcile
   * schedulers from a code-side registry on every restart.
   */
  schedulerActions?: SchedulerActionHandlers;
}

/**
 * Host-provided actions for editing scheduled repeatable jobs.
 *
 * The Workbench UI dispatches to these callbacks via dedicated API routes
 * (PATCH/DELETE/POST under `/schedulers/repeatable/:queueName/:jobName`).
 * Implementations should be idempotent and durable — they're expected to
 * persist the change in the host's own store (so it survives reconcile
 * passes) and propagate to BullMQ.
 */
export interface SchedulerActionHandlers {
  /**
   * Apply an interval-preset change and/or enabled toggle to a single
   * scheduled job, identified by its BullMQ queue name + job name.
   *
   * `intervalPreset === null` means "reset to code default cron". Implementations
   * should return the resulting state so the UI can show a confirmation.
   */
  onEdit: (input: {
    queueName: string;
    jobName: string;
    ...
    enabled?: boolean;
  }) => Promise<{ status: "enabled" | "disabled"; cron?: string }>;

  /** Disable a scheduler. Equivalent to onEdit({enabled: false}) but unambiguous. */
  onRemove: (input: {
    queueName: string;
    jobName: string;
  }) => Promise<{ status: "disabled" }>;

  /** Trigger a one-off ad-hoc run, returning the enqueued job id. */
  onRunNow: (input: {
    queueName: string;
    jobName: string;
  ...
  /**
   * Curated interval presets to show in the edit dialog. Keep small — the
   * point of presets is to avoid free-form cron (a bad expression is a
   * trivial self-DoS).
   */
  intervalPresets: Array<{ value: string; label: string; cron: string }>;
}
...
/**
 * Capability probe returned from `GET /schedulers/actions`. The UI uses
 * this to decide whether to render the per-row action menu — when the host
 * app hasn't wired `schedulerActions`, the response is `{enabled: false}`
 * and the actions stay hidden.
 */
export interface SchedulerActionsConfig {
  enabled: boolean;
  intervalPresets: Array<{ value: string; label: string; cron: string }>;
}
