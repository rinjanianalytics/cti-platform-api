...
export const api = {
  ...
  /**
   * Probe for scheduler action support. When the host hasn't wired
   * `schedulerActions`, the response is `{enabled: false}` and the UI
   * hides the action menu — direct BullMQ writes would only be reverted
   * by the host's next reconcile pass anyway.
   */
  async getSchedulerActionsConfig(): Promise<{
    enabled: boolean;
    intervalPresets: Array<{ value: string; label: string; cron: string }>;
  }> {
    return fetchJson(`${API_BASE}/schedulers/actions`);
  },

  async editRepeatableScheduler(
    queueName: string,
    jobName: string,
    body: { intervalPreset?: string | null; enabled?: boolean },
  ): Promise<{ status: "enabled" | "disabled"; cron?: string }> {
    return fetchJson(
      `${API_BASE}/schedulers/repeatable/${encodeURIComponent(queueName)}/${encodeURIComponent(jobName)}`,
      { method: "PATCH", body: JSON.stringify(body), headers: { "Content-Type": "application/json" } },
    );
  },

  async removeRepeatableScheduler(
    queueName: string,
    jobName: string,
  ...
  async runRepeatableSchedulerNow(
    queueName: string,
    jobName: string,
  ): Promise<{ jobId: string }> {
    return fetchJson(
      `${API_BASE}/schedulers/repeatable/${encodeURIComponent(queueName)}/${encodeURIComponent(jobName)}/run`,
      { method: "POST" },
    );
  },
