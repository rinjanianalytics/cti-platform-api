...
/**
 * Probe for whether the host wired `schedulerActions`. Cached infinitely
 * (capability doesn't change at runtime). When `enabled: false` the
 * Schedulers UI should hide the action menu.
 */
export function useSchedulerActionsConfig() {
  return useQuery({
    queryKey: ["schedulers", "actions-config"] as const,
    queryFn: () => api.getSchedulerActionsConfig(),
    staleTime: Number.POSITIVE_INFINITY,
  });
}

export function useEditRepeatableScheduler() {
  ...
  return useMutation({
    mutationFn: (input: {
      ...
      api.editRepeatableScheduler(input.queueName, input.jobName, {
...
export function useRemoveRepeatableScheduler() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: { queueName: string; jobName: string }) =>
      api.removeRepeatableScheduler(input.queueName, input.jobName),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.schedulers.all });
    },
  });
...
export function useRunRepeatableSchedulerNow() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (input: { queueName: string; jobName: string }) =>
      api.runRepeatableSchedulerNow(input.queueName, input.jobName),
    onSuccess: () => {
      // Active/recent jobs may shift after firing an ad-hoc run.
